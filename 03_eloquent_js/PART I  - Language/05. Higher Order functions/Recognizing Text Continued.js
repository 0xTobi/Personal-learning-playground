...
/* Explanation
   A typeScripts function that accepts a piece of text as an argument.

  function textScripts(text) {
    let scripts = countBy(text, char => {                             ---   The countBy function is called with "text" and "char" as arguments and is stored inside the "scripts" variable   ---   The text gotten from the typescript function is passed to the countBy function. The text is then iterated over and each elements(letter) passed as an argument to the "char" function value.
      let script = characterScript(char.codePointAt(0));              ---   The characterScript function is called within the scope of the countBy function, and is stored inside the "script" variable   ---   The codePointAt method now checks the element(letter) to see its script code. The script code is now passed as an argument to the characterScript function, which checks the SCRIPTS array to see what script.range it falls under. If it finds a script with such range then it returns that script object, and if it does'nt find any it returns "null".
      return script ? script.name : "none";                           ---   If a script match is found for the script code then return the scripts name, else return the string "none".

          function countBy(items, groupName) {                        ---   (text, char => {let script = characterScript(char.codePointAt(0)); return script ? script.name : "none"})
            let counts = [];                                          
            for (let item of items) {                                 ---   Iterates over the characters of the text. 
                let name = groupName(item);                           ---   name = char => ...(character)   ---   The codePointAt method now checks the element(character) to see its script code. The script code is now passed as an argument to the characterScript function, which checks the SCRIPTS array to see what script.range it falls under. If it finds a script with such range then it returns that script object, and if it does'nt find any it returns "null". If a script match is found for the script code then return the scripts name, else return the string "none"....... So the name variable holds the script name.
                let known = counts.findIndex(c => c.name == name);    ---   From the first iteration, after checking the name of the script the character is written in, it checks the counts array to compare the value of the c.name with the value of the current script name to see if they match, but there is no script yet in the counts array, so the the findIndex method returns "-1" and the "if" branch is taken. The name is passed into the counts array, with a "count" property which is initially set to 1. Then it moves on to the next iteration, the second character. It checks the name of the script the character is written in, if the name is already in the counts array then the else branch is taken and it simply adds "1" to the current count value. Then it again moves to the next iteration, if it finds a script name that is not yet included in the counts array, then the if branch is taken, which adds the name of the scripr and a count property whose value is initially set to 1.
                if (known == -1) { 
                    counts.push({name, count: 1});
                } else { 
                    counts[known].count++; 
            ...
            return counts;                                            ---   Return the counts arrray.   E.g   ---   counts = [{name: "latin", counts: 7}, {name: "greek", count: 3}, {name: "none", counts: 10}, {name: "spanish", counts: 9}]
          }

    }).filter(({name}) => name != "none");                            ---   This filter method filters out entries whose name are not equal to "none" into a new array.

    let total = scripts.reduce((n, {count}) => n + count, 0)          ---   Sums up the count property value, and saves the resulting value in the total variable.
    if (total == 0) return "No scripts found";                        ---   If total is equal to zero, that means no matching scripts was found for the text in our scripts database.

    return scripts.map(({name, count}) => {                           ---   Returns a new array using the .map array method to create a new array with the name and count property value.
      return `${Math.round(count * 100 / total)}% ${name}`;           ---   Calculates the percentage composition of the scripts that makes up the text, and also displays their name in front.
    }).join(", ");                                                    ---   join method is an array method that combines strings together. Without the .join(", ") we would get something like this ['53% Han', '27% Latin', '20% Cyrillic']....
}
