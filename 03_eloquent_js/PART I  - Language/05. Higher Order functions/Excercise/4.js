...
function writingDirection(text) {
    let scripts = countBy(text, char => { 
        ...
        return script ? script.direction : "none"       // Returns the direction of the scripts that make up the text
    }).filter(({name}) => name != "none");
    
    return scripts.reduce((a, b) => a.count > b.count ? a.name : b.name);   // Compares the writing directions to see which has a greater value. 

}

console.log(writingDirection('英国的狗 "woof", 俄 斯的狗 "тяв" 𞤑𞤵𞥅𞤤𞤢𞤤𞥒𞤘𞤮𞥅𞤼𞤮𞤳𞤢𞤤𞤢'))
