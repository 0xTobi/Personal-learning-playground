...
// finding the largest script without Higher Order Function.
...
// Code that finds the average year of the living and dead scripts in the data set using higher order array methods.
    function average(array) {
    return array.reduce((total, element) => total + element) / array.length;  // Here, initially total = first element, and element starts iterating from the second element in the array. It adds the second element to the total value, and whatever is obtained is saved as the new total value. The other elements follow suite, increading the size of the total value.
    }
    ...
    // Explanation
    // Math.round - Rounds up the value gotten from the average function.
    // Note!!! Our average function only requires the year property from each script in the SCRIPTS array, first from scripts that are still living then those that are dead.
    // So to seperate the living and dead scripts: We use the filter array method to get the scripts that returns a true script.living value and also a those that returns a false script.living value.
    // Note!! The filter returns script objects but we only need the script.year value to calculate the average. So we use the map array method to extract only the script.year values.
    // We now have the isolated script.year values so the average function can add the up, and then divide them by the total number of elements to get the average.
    

// Using for loop.
    let total = 0, count = 0;
    for (let script of SCRIPTS) {
        if (script.living) {    // If script.living is true.
            total = total + script.year;
            count = 1;
        }
    }
    ...
    for (let script of SCRIPTS) {
        if (!script.living) {    // If script.living is false.
            total2 = total2 + script.year;
            count2 = count2 + 1;
        }
    }
